...
JwtAuthFilter::JwtAuthFilter() {
    // 从应用程序配置中获取JWT密钥
    auto& app = drogon::app();
    m_jwtSecret = app.getCustomConfig()["jwt_secret"].asString();
    
    if (m_jwtSecret.empty()) {
        std::cerr << "警告: JWT密钥未配置或为空" << std::endl;
    }
...
void JwtAuthFilter::doFilter(const drogon::HttpRequestPtr &req,
    ...
    try {
        ...
        // 将用户信息添加到请求属性中
        req->addAttribute("user_uuid", result.userUuid);
        req->addAttribute("username", result.username);
        req->addAttribute("is_admin", result.isAdmin);
